  ...
  return (
    <section className="relative bg-black text-white pt-20 pb-12 md:pt-24 md:pb-16 px-8 border-t border-white/5 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 md:grid-cols-3 gap-3 sm:gap-6 md:gap-8">
          {skills.map((skill, index) => (
            <motion.div
              ...
              className="group relative flex min-w-0 items-center gap-2 sm:gap-4 p-3 sm:p-6 rounded-2xl border border-white/10 bg-white/[0.02] hover:bg-white/5 transition-all duration-300"
            >
              ...
              <div className={`shrink-0 p-2 sm:p-3 rounded-xl bg-white/5 group-hover:bg-white/10 transition-colors duration-300 ${skill.color}`}>
                <skill.icon className="w-6 h-6 sm:w-8 sm:h-8" />
              </div>
              
              <span className="min-w-0 flex-1 font-medium text-sm sm:text-base leading-snug text-white/80 group-hover:text-white transition-colors duration-300 break-words">
                {skill.name}
              </span>
            </motion.div>
          ))}
